...
class Method {
	...
	@Keyword
	def TextDegerlerindenBirineEsitElementVarMi(TestObject object, String text1, String text2, String text3) {
		WebDriver driver = com.kms.katalon.core.webui.driver.DriverFactory.getWebDriver()
		WebDriverWait wait = new WebDriverWait(driver, 10);
		boolean a=false
		List<WebElement> elements = WebUI.findWebElements(object,10)
		for (element in elements) {
			if(element.getText().replaceAll("\\s+", "").contains(text1.replaceAll("\\s+", "")) || element.getText().replaceAll("\\s+", "").contains(text2.replaceAll("\\s+", "")) || element.getText().replaceAll("\\s+", "").contains(text3.replaceAll("\\s+", ""))){
				a=true
				break
			}
		}
	...
	def isimliDosyaIndirildiMi(String dizin,String fileName) {
		...
		for (File file : files) {
			...
			GlobalVariable.FileName = file.getName()
